...
static int writeContinuousExfatChain(BufferedFsWriter &dev, const u32 start, const u32 length)
{
	...
	{
		const int res = dev.write(reinterpret_cast<u8*>(&i), 4);
		if(res != 0) return res;
	}

	const u32 eof = EXFAT_EOF;
	return dev.write(reinterpret_cast<const u8*>(&eof), 4);
}

// Warning, this function relies on the current buffer position in dev!
// Count must be >=1.
// TODO: This breaks horribly on multiple calls with counts not a multiple of 32!
static int writeContinuousBitmapRange(BufferedFsWriter &dev, u32 count)
{
	do
	{
		const u32 bits = (count > 32 ? 32 : count);
		const u32 bitmap = 0xFFFFFFFFu>>(32u - bits);
		const int res = dev.write(reinterpret_cast<const u8*>(&bitmap), 4);
		if(res != 0) return res;
...
int makeFsExFat(const FormatParams &params, BufferedFsWriter &dev, const std::u16string &label)
{
	...
	const u32 bitmapClus        = util::divCeil(clusterCount, bitsPerClus);
	const u32 upCaseClus        = util::divCeil(sizeof(upCaseTable), bytesPerClus);
	...
	// OEM Parameters.
	FlashParameters *const flashParams = reinterpret_cast<FlashParameters*>(&bootRegion[bytesPerSec * 9]); // TODO: If available get this from existing exFAT volume as per spec.
	...
	res = dev.fillAndWrite(reinterpret_cast<const u8*>(reservedEnt), curOffset, 8); // First (reserved) + second (EOF) entry.
	if(res != 0) return res;

	res = writeContinuousExfatChain(dev, 0, bitmapClus);                            // Bitmap cluster chain.
	if(res != 0) return res;

	res = writeContinuousExfatChain(dev, bitmapClus, upCaseClus);                   // Up-case Table cluster chain.
	if(res != 0) return res;

	res = writeContinuousExfatChain(dev, bitmapClus + upCaseClus, 1);               // Root directory cluster chain. TODO: Is 1 cluster always safe? Minimum size is 512 bytes.
	if(res != 0) return res;
	...
	res = writeContinuousBitmapRange(dev, bitmapClus + upCaseClus + 1); // TODO: Is 1 cluster always safe for root dir? Minimum size is 512 bytes.
	if(res != 0) return res;

	// Up-case Table.
	curOffset += secPerClus * bytesPerSec * bitmapClus;
	res = dev.fillAndWrite(reinterpret_cast<const u8*>(upCaseTable), curOffset, sizeof(upCaseTable));
	if(res != 0) return res;
	...
	memcpy(entries[0].label.volumeLabel, label.c_str(), 2 * label.length());

	entries[1].entryType           = TYPE_BITMAP;
	// entries[1].bitmap.flags cleared to zero.
	entries[1].bitmap.firstCluster = EXFAT_FIRST_ENT;
	entries[1].bitmap.dataLength   = util::divCeil(clusterCount, 8u);
	...
	entries[2].upCase.dataLength    = sizeof(upCaseTable);

	curOffset += secPerClus * bytesPerSec * upCaseClus;
	res = dev.fillAndWrite(reinterpret_cast<const u8*>(entries), curOffset, sizeof(entries));
	if(res != 0) return res;

	// Fill remaining directory entries.
	curOffset += secPerClus * bytesPerSec;
	return dev.fill(curOffset); // TODO: Test this against SDFormatter.
}
